...
class IPTopo(Topo):
    ...
    def build(self, *args, **kwargs):
        ...
        for o in self.overlays:
            if not o.check_consistency(self):
                lg.error("Consistency checks for", str(o), "overlay have failed!\n")
    ...
    def addRouters(
        self, *routers: str | tuple[str, dict[str, Any]], **common_opts
    ) -> list["RouterDescription"]:
    ...
    def addLink(
        self, node1: str, node2: str, port1=None, port2=None, key=None, **opts
    ) -> "LinkDescription":
        ...
        key = super().addLink(
            node1=node1, node2=node2, port1=port1, port2=port2, key=key, **opts
        )
    ...
    def hosts(self, sort=True) -> list["HostDescription"]:
        ...
        return [h for h in super().hosts(sort) if not self.isRouter(h)]

    def routers(self, sort=True) -> list["RouterDescription"]:
        """Return a list of router node names"""
        return [
            RouterDescription(n, self) for n in self.nodes(sort) if self.isRouter(n)
        ]
