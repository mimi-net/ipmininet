...
class IPHost(IPNode):
    ...
    def setDefaultRoute(self, intf: IPIntf | str | None = None,
                        v6=False):
        ...
        cmd = f"ip -{version} route del default;" \
              f" ip -{version} route add default {params}"
        ...
        for itf in realIntfList(self):
            for r in itf.broadcast_domain.routers:
                if self.use_v4 and self.use_v4 and len(r.addresses[4]) > 0:
                    self.setDefaultRoute(f"dev {itf.name} via {r.ip}",
                                         v6=False)
                    found = True
                if self.use_v6 and self.use_v6 and len(r.addresses[6]) > 0 \
                    ...
                    self.setDefaultRoute(f"dev {itf.name} via {r.ip6}",
                                         v6=True)
                    found = True
                break
            if found:
