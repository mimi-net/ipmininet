...
def build_bgp_route(
    ...
    :param their_as: the route generated by this function will be injected to the
                     AS having the ASN referenced by this parameter. Notably, this
                     parameter (their_as) is used to not generate an AS-Path
                     containing "their_as" in the AS list
        ...
        communities = BGPAttribute(
            ...
            ExaList(
                [
                    f"{j}:{k}"
                    for j, k in zip(
                        rnd_list(24, True), rnd_list(24, True), strict=False
                    )
                ]
            ),
        )
...
def gen_ip_prefix(family: str):
    ...
    assert family in {"ipv4", "ipv6"}, f'Family "{family}" is not a valid IP family'
    ...
    _CONF_FAMILY = {
        "ipv4": {
            ...
            "end_ip": _233_255_255_255,  # 223.255.255.255 (avoid class D/E addresses)
        },
        "ipv6": {
            "pfx_lo": 16,
            "pfx_hi": 128,
        ...
        if addr is not None:
            for node in addr:
                assert node in {"as1", "as2"}, (
                    f'Unrecognized node: "{node}". Expected: "as1" or "as2"'
                )
                for afi in addr[node]:
                    assert afi in {"ipv4", "ipv6"}, (
                        f'Unrecognized AFI "{afi}" for "{node}" node. '
                    ...
                    if isinstance(addr[node][afi], str):
                        ...
                        assert isinstance(
                            ...
                            "Bad type "
                            f"'{type(addr[node][afi])}' for {afi} AFI. "
                            "Expected: IPv4Interface or IPv6Interface or str."
                        )
