...
def build_bgp_route(ip_networks: Sequence["IPv4Network"] | Sequence["IPv6Network"], my_as: int, their_as: int):
    """
    ...
    for network in ip_networks:
        ...
        communities = BGPAttribute("community",
                                   ExaList(["%d:%d" % (j, k) for j, k in zip(rnd_list(24, True), rnd_list(24, True), strict=False)]))
...
def gen_ip_prefix(family: str):
    ...
    def random_prefix(afi):
        ...
        return ip_network(f"{ip_address(int_addr)}/{mask_len}"
                          ,
                          strict=False)
...
class ExaBGPTopoInjectPrefixes(IPTopo):
    ...
    def __init__(self, routes: dict[str, Sequence["BGPRoute"]] | None = None,
                 addr: dict[str, dict[str, Union[str, "IPv4Interface", "IPv6Interface"]]] | None = None,
                 *args, **kwargs):
        """
        ...
        if addr is not None:
            for node in addr:
                assert node in {"as1", "as2"}, f'Unrecognized node: "{node}". Expected: "as1" or "as2"'
                for afi in addr[node]:
                    assert afi in {"ipv4", "ipv6"}, f'Unrecognized AFI "{afi}" for "{node}" node. ' \
