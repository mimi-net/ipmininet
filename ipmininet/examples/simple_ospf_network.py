...
class SimpleOSPFNet(IPTopo):
    ...
    def build(self, *args, **kwargs):
        ...
        """
        # Build backbone
        r1, r2, r3 = self.addRouters("r1", "r2", "r3", use_v4=True, use_v6=False)
        self.addLink(r1, r2)
        self.addLink(r1, r3, igp_metric=5)
        self.addLink(r3, r2)
        for r in (r1, r2, r3):
            for i in range(HOSTS_PER_ROUTER):
                self.addLink(r, self.addHost("h%s%s" % (i, r)), params2={"v4_width": 5})
        ...
        for r in (r6, r7):
            for i in range(HOSTS_PER_ROUTER):
                self.addLink(r, self.addHost("h%s%s" % (i, r)), igp_area="2.2.2.2")
