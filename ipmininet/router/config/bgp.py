...
def filter_allows_all_routes(a: "RouterDescription", b: "RouterDescription"):
    ip4_pfxl = PrefixList(
        name="hello-world-v4",
        family="ipv4",
        entries=(PrefixListEntry("0.0.0.0/0", le=32),),
    )
    ip6_pfxl = PrefixList(
        name="hello-world-v6", family="ipv6", entries=(PrefixListEntry("::/0", le=128),)
    )
    ...
    a.get_config(BGP).filter(
        policy=PERMIT,
        from_peer=b,
        matching=(ip4_pfxl, ip6_pfxl),
        order=RouteMap.DEFAULT_POLICY,
    ).filter(
        policy=PERMIT,
        to_peer=b,
        matching=(ip4_pfxl, ip6_pfxl),
...
def ebgp_session(
    ...
    if link_type:
        ...
        if link_type == SHARE:
            ...
            a.get_config(BGP).set_community(
                1, from_peer=b, matching=(all_al_v4, all_al_v6)
            ).set_local_pref(
                150,
                from_peer=b,
                matching=(
            ...
            b.get_config(BGP).set_community(
                1,
                from_peer=a,
                matching=(
                    all_al_v4,
            ...
            a.get_config(BGP).deny(to_peer=b, matching=(up_link,), order=10).deny(
                to_peer=b, matching=(peers_link,), order=15
            ).permit(to_peer=b, order=20)

            b.get_config(BGP).deny(to_peer=a, matching=(up_link,), order=10).deny(
                to_peer=a, matching=(peers_link,), order=15
...
class BGPConfig:
    ...
    def rm_name(peer: str, family: str, direction: str):
        return f"{peer}-{family}-{direction}".format(
            peer=peer, family=family, direction=direction
        )
    ...
    def set_community(
        ...
        if to_peer is not None:
            self.add_set_action(
                peer=to_peer,
                set_action=RouteMapSetAction("community", community),
                matching=matching,
                direction="out",
                name=name,
            )
    ...
    def filter(
        ...
        from_peer: str | None = None,
        to_peer: str | None = None,
        matching: Sequence[AccessList | CommunityList | PrefixList] = (),
        order=10,
    ) -> "BGPConfig":
    ...
    def filters_to_match_cond(
        self,
        filter_list: Sequence[AccessList | CommunityList | PrefixList],
        family: str,
    ):
        match_cond = []
        ...
        for f in filter_list:
            if isinstance(f, CommunityList):
                ...
                if f.family == family:
                    match_cond.append(
                        RouteMapMatchCond("access-list", f.name, f.family)
                    )
                ...
                if f.family == family:
                    match_cond.append(
                        RouteMapMatchCond("prefix-list", f.name, f.family)
                    )
    ...
    def add_set_action(
        ...
        for family in ("ipv4", "ipv6"):
            ...
            rm_entry = RouteMapEntry(
                family=family, match_cond=match_cond, set_actions=[set_action]
            )
...
class AbstractBGP(ABC, RouterDaemon):
    ...
    def _build_afi(self, af: list["AddressFamily"] | None):
        afis = self.afi
        if af:
    ...
    @staticmethod
    def _address_families(
        af: list["AddressFamily"], nei: list["Peer"]
    ) -> list["AddressFamily"]:
...
class BGP(QuaggaDaemon, AbstractBGP):
    ...
    def build_route_map(self, neighbors: Sequence["Peer"]) -> list[RouteMap]:
        ...
        def find_neighbor(route_map: "RouteMap"):
            ...
            """
            assert route_map.neighbor is not None, (
                "BGP Neighbor route maps must have one neighbor!"
            )
...
class AddressFamily:
    ...
    def __init__(
        self,
        af_name: str,
        redistribute: Sequence[str] = (),
        networks: Sequence[str | IPv4Network | IPv6Network] = (),
        routes=(),
    ):
        self.name = af_name
...
class Peer:
    ...
    def _find_peer_address(
        base: "Router", peer: str, v6=False
    ) -> tuple[str | None, Optional["Router"], str | None]:
        """
        ...
        prio_queue: list[Peer.PQNode] = [
            Peer.PQNode((0, i), to_visit[i]) for i in to_visit
        ]
                ...
                if n.node.asn == base.asn or not n.node.asn:
                    for i in realIntfList(n.node):
                        ...
                        heapq.heappush(
                            prio_queue,
                            Peer.PQNode(
                                (path_cost + i.igp_metric, i.name), my_interface
                            ),
                        )
        return None, None, None
