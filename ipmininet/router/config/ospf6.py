    ...
    def _build_interfaces(self, interfaces):
        ...
        conf = [
            ConfigDict(
                ...
                priority=i.get(
                    "ospf6_priority", i.get("ospf_priority", self.options.priority)
                ),
                dead_int=i.get(
                    "ospf6_dead_int", i.get("ospf_dead_int", self.options.dead_int)
                ),
                hello_int=i.get(
                    "ospf6_hello_int", i.get("ospf_hello_int", self.options.hello_int)
                ),
                cost=i.igp_metric,
