...
def test_topologydb(topology: type[IPTopo]):
    ...
    try:
        ...
        for node, node_info in db._network.items():
            assert node in net, \
                f"The node {node} in the DB file is not in the network"

            # Check type

            assert "type" in node_info, \
                f"No info on the type of node {node}"
            node_type = node_info["type"]
            if node_type == "host":
                assert isinstance(net[node], IPHost), f"The node {node} is not an " \
                                                      "host"
            elif node_type == "router":
                assert isinstance(net[node], Router), f"The node {node} is not a " \
                                                      "router"
            elif node_type == "switch":
                assert isinstance(net[node], IPSwitch), f"The node {node} is not a " \
                                                        "switch"
            else:
            ...
            for info_key, info_value in node_info.items():
                if info_key in {"type", "interfaces"}:
                    continue
