...
@pytest.mark.parametrize(
    "named_cfg,zone_args,exp_named_cfg,exp_zone_cfg",
    [
        ({}, {}, [], []),
        ({"log_severity": "info"}, {}, ["        severity info;"], []),
...
def test_dns_network(named_cfg, zone_args, exp_named_cfg, exp_zone_cfg):
    try:
        ...
        assert_dns_record(
            net["master2"],
            "localhost",
            AAAARecord("master2.test.org", net["master2"].defaultIntf().ip6),
...
def test_zone_delegation():
    try:
        ...
        records = [
            ...
            (
                [net["orgdns"]],
                org_delegation_records + mydomain_delegation_records + root_hints,
            ),
...
@require_root
@pytest.mark.parametrize("topo", [StaticRoutingNet, DNSNetwork, SimpleBGPTopo])
