...
# mount stack (with a private /tmp). Needed so concurrent IPMininet test
# networks cannot interfere: bridges/veths/iptables (netns), cleanup()'s
# pkill/pgrep (PID ns) and node configs/zebra sockets under /tmp (mount ns)
# are all scoped per worker. Invoked by execnet as: <this> -u [-c] bootstrap
set -euo pipefail
